...
//    }
    ...
    {
//        if (empty($this->_validationFunc)) {
//            return $element;
//        }
        $element = apply_filters('wptoolset_form_submit_' . $this->_id, $element);
        if (isset($element['error'])) {
            $this->_errors = true;
            $element['#error'] = '';
            foreach ($element['error'] as $error) {
                $element['#error'] .= $error->get_error_message();
            }
        }
//        $check = call_user_func_array($this->_validationFunc, array($element['#validate'], $element['#value']));
    ...
    public function renderElement($element)
    {
        ...
        if (is_callable(array($this, $method))) {
            ...
            if (!empty($element['#validate'])) {
                do_action('wptoolset_forms_field_html_validation_' . $this->_id, $element);
